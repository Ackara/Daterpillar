    ...
    public class SqlScriptBuilderTest
    {
        ...
        private void AppendSchema(IScriptBuilder builder, IDbConnection connection, [CallerMemberName]string testName = null)
        {
            using (connection)
            {
                ...
                regex = new Regex(@"generated by: \w+", RegexOptions.IgnoreCase);
                bool createdWasAppended = regex.IsMatch(content);
                content = (createdWasAppended ? regex.Replace(content, "GENERATED BY: MSTest") : content);
